...
def cve(option: tuner.Option, ver_major: int, ver_minor: int, ver_micro: int) -> None:
    fp.subheader_print("CVE Security Recommendations", option)
    if not option.cve_file or not os.path.isfile(option.cve_file):
        fp.info_print("Skipped due to --cve-file option undefined", option)
        return None

    cve_found: int = 0
    with open(option.cve_file, mode="r", encoding="utf-8") as cf:
        for line in cf:
            cve_: typ.Sequence[str] = line.split(";")
            ...
            ver_compare: str = " ".join((
                ...
                ":",
                "<=" if (ver_major, ver_minor, ver_micro) <= (cve_ver_major, cve_ver_minor, cve_ver_micro) else ">"
            ))
    ...
    if cve_found == 0:
        fp.good_print("NO SECURITY CVE FOUND FOR YOUR VERSION", option)
        return None

    if (ver_major, ver_minor) == (5, 5):
        fp.info_print("False positive CVE(s) for MySQL and MariaDB 5.5.x can be found.", option)
        fp.info_print("Check careful each CVE for those particular versions", option)
